...
En el presente capítulo se contextualiza la detección de anomalías en servicios de televisión por protocolo de internet (TV-over-IP) \citep{itu_iptv_overview}. Además, se revisa el estado del arte y se enuncian los objetivos del trabajo.

\section{Servicios de TV-over-IP y desafíos operativos}
...
\begin{figure}[ht]
	...
	\caption{Ejemplo conceptual de una métrica de servicio con un tramo de comportamiento anómalo. Elaboración propia.}
...
La propuesta de este trabajo se acopla al \textit{stack} Prometheus--Grafana--Opsgenie \citep{salituro2020grafana, grafana_site, opsgenie_site} ya adoptado en la organización. Así se busca un equilibrio entre sensibilidad y falsos positivos calibrable con el equipo de operaciones.

La tabla~\ref{tab:comparativa-enfoques} resume familias de enfoques y su relación típica con datos y métricas de evaluación. 

Para fines orientativos, Malhotra et al. \citep{malhotra2015long} entrenaron un encoder-decoder LSTM sobre series multivariadas de sensores y utilizaron el error de reconstrucción como señal de anomalía; Liu et al. \citep{liu2008isolation} aplicaron \textit{isolation forest} sobre vectores de características con puntaje de anomalía; en esta memoria se adoptó un autoencoder LSTM sobre telemetría operativa de TV-over-IP con error de reconstrucción y umbral calibrado. Ninguno de estos resúmenes pretende ser exhaustivo, sino ubicar la propuesta en el panorama habitual de ingeniería de confiabilidad y aprendizaje automático.
...
\toprule
\textbf{Enfoque} & \textbf{Tipo de modelo o regla} & \textbf{Datos típicos} & \textbf{Métricas habituales} \\
\midrule
Umbrales fijos & Reglas fijas & Series univariadas & Tiempo de detección alto, fasas alertas \\
Modelos clásicos & kNN, Isolation Forest, etc. & Ventanas vectorizadas & Precisión, \textit{recall}, F1 \\
Modelos secuenciales profundos & Autoencoder LSTM, RNN & Ventanas multivariadas & Error de reconstrucción, F1 \\
Productos comerciales & Algoritmos propietarios & Telemetría agregada & Integración con el proveedor \\
\bottomrule
\end{tabular}
\end{table}
...
El objetivo general del trabajo consistió en diseñar e implementar un sistema inteligente capaz de detectar anomalías en tiempo de operación, compatible con el monitoreo existente en un servicio de TV-over-IP basado en API, aprovechando la integración con sistemas de monitoreo y alertas para mejorar la capacidad de reacción frente a incidentes. Esto permite, por otra parte, evitar la dependencia de umbrales estáticos o problemas de seguridad al utilizar servicios de terceros.
