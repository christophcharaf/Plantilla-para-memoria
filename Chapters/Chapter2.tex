...
La implementación del autoencoder LSTM se apoyó en TensorFlow y su interfaz Keras \citep{abadi2016tensorflow, chollet2015keras}, que ofrecen una capa de alto nivel para definir, entrenar y serializar redes neuronales profundas. Entre las capas utilizadas figuran LSTM, RepeatVector y TimeDistributed, componentes nativos que permiten expresar la arquitectura codificador--decodificador con pocas líneas. La elección del \textit{framework} obedeció a la integración estable con el ecosistema Python usado para procesamiento de datos, a la portabilidad del modelo entrenado a contenedores en CPU y al formato de serialización \texttt{.weights.h5}, que se acompaña de un archivo \texttt{JSON} con la definición estructural del grafo para reconstruir el modelo en inferencia sin acoplarse a una versión particular del entorno de entrenamiento.

Para el preprocesamiento se utilizó scikit-learn \citep{pedregosa2011scikit}. La normalización se realizó con MinMaxScaler, configurado con cotas fijas para desacoplar la transformación de la distribución particular de los datos de entrenamiento. La persistencia del preprocesador, necesaria para garantizar la misma transformación entre entrenamiento e inferencia, se gestionó con joblib \citep{joblib_site}, biblioteca de serialización binaria habitual en ese ecosistema.

La manipulación de las series temporales se apoyó en NumPy \citep{harris2020numpy} y pandas \citep{mckinney2010pandas}, estándares en Python para arreglos n-dimensionales y tablas indexadas por tiempo. La obtención de métricas desde Prometheus se realizó vía HTTP con la biblioteca requests \citep{requests_site}, mientras que la instrumentación del servicio simulado utilizó el cliente oficial prometheus\_client \citep{prometheus_client_site}, que expone contadores e histogramas en el formato esperado por el recolector. La carga de archivos de configuración en formato \texttt{YAML} se delegó en PyYAML \citep{pyyaml_site}, y la generación de gráficos de evaluación \textit{offline} se realizó con matplotlib y seaborn \citep{matplotlib_site, seaborn_site}. 
...
prometheus\_\allowbreak client & 0.17 & Exposición de métricas en el servicio simulado. \\
\addlinespace
...
En la práctica, las dependencias se agruparon en tres bloques funcionales y modulares: cómputo numérico y aprendizaje profundo (TensorFlow, NumPy, pandas, scikit-learn), observabilidad (requests, prometheus\_client, PyYAML) y evaluación visual (matplotlib, seaborn). Esta separación facilitó auditar qué componentes intervienen en el entrenamiento, en la inferencia y en los ensayos documentados en el capítulo de resultados.
...
\midrule
Lenguaje y environment & Python 3.10 (\path{python:3.10-slim}). \\
Orquestación & Docker Compose, perfil \texttt{dev}. \\
Uso de recursos & 1 CPU, 2~GB RAM. \\
Aceleración por GPU & No (solo CPU). \\
Servicios auxiliares & Prometheus, Grafana, servicio simulado. \\
Red & Puente \texttt{monitoring}, sin exposición pública. \\
Destino de deploy & AWS propia, sin analítica de terceros. \\
\bottomrule
\end{tabular}
\end{table}
...
La evaluación del sistema combinó dos planos complementarios. En el primero se valoró la calidad de reconstrucción del autoencoder, base del aprendizaje no supervisado, mediante el error cuadrático medio (MSE) \citep{chollet2015keras} como función de pérdida y señal escalar de anomalía, y el error absoluto medio (MAE) como métrica auxiliar de monitoreo. 

En el segundo se valoró la capacidad de detección sobre ventanas con anomalías sintéticas inyectadas, y para ello se emplearon métricas de clasificación binaria habituales \citep{pedregosa2011scikit}: precisión, \textit{recall}, \textit{F1-score} y tasa de falsos positivos. 

La distinción entre ambos planos respondió al carácter no supervisado del entrenamiento: las métricas de clasificación solo pudieron computarse en escenarios controlados con anomalías conocidas, mientras que el error de reconstrucción se midió de forma continua sobre tramos nominales.

Las métricas utilizadas y el propósito que cumplieron en la validación fueron los siguientes:

\begin{itemize}
\item MSE: error cuadrático medio entre la ventana de entrada y su reconstrucción; función de pérdida del entrenamiento y señal escalar de anomalía en inferencia.
\item MAE: error absoluto medio entre entrada y reconstrucción; métrica auxiliar de monitoreo durante el entrenamiento, menos sensible a valores atípicos.
\item Precisión: fracción de ventanas marcadas como anómalas que efectivamente lo eran; controla la proporción de falsas alertas elevadas al servicio de guardia.
\item Recall: fracción de anomalías reales correctamente detectadas; mide la cobertura de incidentes ante anomalías sintéticas inyectadas.
\item F1-score: media armónica de precisión y exhaustividad; resume en un único valor el compromiso entre falsas alertas y cobertura.
\item Tasa de falsos positivos: fracción de ventanas nominales marcadas erróneamente como anómalas; verifica la estabilidad del umbral sobre tramos de régimen nominal.
\end{itemize}
