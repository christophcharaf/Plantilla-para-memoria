...
En este capítulo se describe el diseño e implementación del sistema de detección de anomalías: el pipeline de datos desde Prometheus hasta las ventanas multivariadas, la arquitectura del autoencoder LSTM, el procedimiento de entrenamiento y calibración del umbral, y la integración del servicio de inferencia con Opsgenie y Grafana.
...
Se implementó como un conjunto de módulos Python invocados desde el script de entrenamiento (\path{scripts/train.py}) y reutilizados en inferencia (\path{scripts/inference.py}), de modo que entrenamiento y producción comparten exactamente las mismas reglas de transformación.

La configuración operativa, incluidas las consultas PromQL, las cotas de normalización, el tamaño de ventana y los parámetros de alerta, se externalizó en archivos YAML bajo \path{config/}, lo que permitió ajustar el comportamiento sin modificar el código fuente.

\subsection{Recolección y alineación temporal}

La primera etapa obtiene series desde la API HTTP de rango de Prometheus (\path{api/v1/query\_range}).

Un cliente dedicado (\texttt{PrometheusClient}) lee las consultas definidas en \path{config/data.yaml} y ejecuta cada consulta con paso de muestreo de 30~segundos.
...
Cuando faltaban puntos aislados dentro del intervalo de muestreo, se aplicó \textit{forward-fill} acotado antes de descartar ventanas incompletas. El cliente incorporó además un ajuste automático del paso de consulta cuando el rango solicitado superaba el límite de 11\,000 puntos por petición impuesto por Prometheus y preservó la cobertura temporal sin truncar silenciosamente el historial.

Para el entrenamiento sobre el entorno autorizado de la empresa, el cliente apuntó a la instancia Prometheus interna. En el repositorio reproducible y en el perfil \texttt{dev} de Docker Compose, las mismas consultas se ejecutaron contra un servicio simulado instrumentado con \path{prometheus\_client}, compartiendo núcleo de generación con el generador sintético offline (\path{traffic\_simulation\_core.py}).
...
\midrule
\texttt{request\_rate} & \texttt{rate(http\_requests\_\allowbreak total[5m])} & \texttt{sum} & Demanda instantánea sobre el servicio \\
\texttt{latency\_p95} & \texttt{histogram\_quantile(0.95,}\allowbreak\texttt{ rate(...[5m]))} & \texttt{max} & Cola superior de tiempos de respuesta \\
\texttt{memory\_usage} & \texttt{process\_resident\_\allowbreak memory\_bytes\{...\}} & \texttt{mean} & Presión de memoria del proceso \\
\texttt{error\_rate} & \texttt{rate(http\_request\_\allowbreak errors\_total[5m])} & \texttt{sum} & Tasa de respuestas erróneas \\
\texttt{cpu\_usage} & \texttt{rate(process\_cpu\_\allowbreak seconds\_total[5m])} & \texttt{sum} & Consumo de CPU del servicio \\
\bottomrule
\end{tabular}
\end{table}
...
La normalización se realizó con MinMaxScaler en modo \texttt{fixed\_minmax}: en lugar de ajustar mínimos y máximos sobre el conjunto de entrenamiento, se fijaron cotas determinísticas por variable en \path{config/data.yaml}, derivadas del análisis del servicio simulado y validadas contra consultas reales. La tabla~\ref{tab:fixed-bounds} resume los rangos adoptados.
...
Las cotas de la tabla~\ref{tab:fixed-bounds} se derivaron del análisis del servicio simulado que se basó en el servicio real como referencia directa:
...
El preprocesador serializado (\path{models/preprocessor.joblib}) persiste el escalador, la lista de columnas activas y la configuración de variables temporales, de modo que inferencia y entrenamiento aplican la misma transformación bit a bit.
...
\begin{itemize}
\item Entrenamiento (\texttt{create\_sequences}): ventanas deslizantes con \textit{stride}~$= 1$, con solapamiento máximo entre muestras consecutivas. Con 90~días de muestreo cada 30~segundos, se produjeron alrededor de $250\,000$ ventanas, frente a unas $12\,000$ obtenidas con stride~$= 20$ en prototipos iniciales.
...
\midrule
Recolección & Consultas PromQL $\rightarrow$ series por métrica & \texttt{PrometheusClient}, \path{config/data.yaml} \\
Alineación & Series $\rightarrow$ marco temporal unificado & \texttt{pandas}, forward-fill acotado \\
Ingeniería & Marco + reloj $\rightarrow$ 11 variables & \texttt{add\_temporal\_features} de \texttt{preprocessor} \\
Normalización & Crudos $\rightarrow$ $[0,1]$ fijo & MinMaxScaler, cotas YAML \\
Ventanas & Serie $\rightarrow$ $(N, 20, 11)$ & \texttt{WindowGenerator} \\
Persistencia & Escalador y metadatos & \path{preprocessor.joblib} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{./Figures/sliding_window.png}
\caption{Estructura lógica de ventanas deslizantes.}
\label{fig:sliding_window}
\end{figure}

\subsection{Organización del código fuente}

El repositorio entregable agrupó la lógica en cuatro paquetes bajo \path{src/}, más scripts ejecutables en \path{scripts/} y configuración en \path{config/}. La tabla~\ref{tab:modulos} resume la responsabilidad de cada módulo principal. Esta separación permitió probar el preprocesamiento y la generación de ventanas de forma aislada antes de integrarlos con el servicio de inferencia.
...
\midrule
\texttt{PrometheusClient} & Consultas HTTP, agregación y alineación de series \\
\texttt{DataPreprocessor} & Variables temporales y normalización \\
\texttt{Windowing} & Ventanas deslizantes para entrenamiento e inferencia \\
\texttt{SyntheticData} & Generación offline de series de respaldo \\
\texttt{LSTMAutoencoder} & Definición, entrenamiento y persistencia del modelo \\
\texttt{Detector} & Puntuación de anomalía por ventana \\
\texttt{OpsgenieClient} & Emisión de alertas y notificaciones de resolución \\
\texttt{GrafanaLinks} & Construcción de URLs contextuales al panel de Grafana \\
\texttt{train.py} & Orquestación del entrenamiento extremo a extremo \\
\texttt{inference.py} & Bucle de detección en tiempo de operación \\
\texttt{tune\_model.py} & Exploración de arquitecturas sin afectar artefactos productivos \\
\bottomrule
\end{tabular}
\end{table}
...
Cualquier divergencia en el orden de transformaciones, en las columnas activas o en la política de ventanas se tradujo en errores de reconstrucción artificialmente elevados y en falsos positivos masivos, fenómeno reproducido al comparar un escalado adaptativo entrenado con datos sintéticos contra consultas reales a Prometheus.Para cerrar esa brecha se adoptaron tres medidas concretas.

Primero, el mismo objeto \texttt{DataPreprocessor} serializado se cargó en inferencia sin reajuste sobre datos recientes. Segundo, las consultas PromQL y las reglas de agregación se leyeron del mismo archivo \path{config/data.yaml} en ambos modos. Tercero, la generación de ventanas en inferencia utilizó exclusivamente los últimos $T$ puntos completos, mientras que en entrenamiento se generó el conjunto completo con stride~$=1$, pero aplicando la misma función de extracción por ventana.

\section{Arquitectura del sistema}
\label{sec:arquitectura-modelo}

El sistema se organizó en capas acopladas al monitoreo existente. Un servicio simulado (en el entorno demostrativo) o la instancia productiva exponían métricas que Prometheus recolectaba periódicamente y consultaba el contenedor de detección. Grafana visualizó las series y Opsgenie recibió las alertas emitidas. La figura~\ref{fig:infra_demo.png} resume esta topología en el perfil \texttt{dev} de Docker Compose.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{./Figures/infra_demo.png}
\caption{Topología de despliegue del sistema de detección.}
\label{fig:infra_demo.png}
\end{figure}

El bloque detector de la figura anterior agrupa el pipeline de datos, el autoencoder LSTM y la lógica de umbral descritos en las secciones anteriores.

La figura~\ref{fig:flujo-detector} detalla el flujo interno de inferencia: preprocesamiento de la ventana, codificación y decodificación, cálculo del error de reconstrucción y comparación con el umbral calibrado; ante una anomalía confirmada, se emiten alertas hacia Opsgenie y enlaces contextuales a Grafana.

\begin{figure}[H]
\centering
\includegraphics[width=0.89\textwidth,height=0.35\textheight,keepaspectratio]{./Figures/flow_diag.png}
\caption{Flujo interno del contenedor de detección: desde la consulta a Prometheus hasta la emisión de alertas.}
\label{fig:flujo-detector}
\end{figure}
...
El codificador apiló tres capas LSTM con 64, 32 y 16 unidades respectivamente. Las dos primeras devolvieron secuencias completas (\texttt{return\_sequences=True}), y la tercera produjo el vector latente de dimensión 16. Entre capas recurrentes se insertó \textit{dropout} del 10~\% para regularizar.

El decodificador expandió el latente mediante RepeatVector, replicándolo a lo largo de los 20 pasos temporales, y aplicó tres capas LSTM simétricas (16, 32 y 64 unidades) con dropout del 10~\%. La salida se obtuvo con una capa \texttt{TimeDistributed}\allowbreak\texttt{(Dense(11))} que reconstruyó las once variables en cada instante. La figura~\ref{fig:lstm-autoencoder-arch} resume la pila de capas y las formas tensoriales intermedias.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth,height=0.38\textheight,keepaspectratio]{./Figures/lstm_autoencoder_arch.png}
\caption{Arquitectura del autoencoder LSTM: codificador, \textit{bottleneck} latente y decodificador.}
\label{fig:lstm-autoencoder-arch}
\end{figure}

La tabla~\ref{tab:capas-modelo} complementa el diagrama con el rol de cada capa.
...
\end{equation}
donde $x$ es la ventana observada y $\hat{x}$ la reconstrucción del modelo. Una ventana se marcó como anómala cuando $e$ superó un umbral $\tau$ calibrado sobre errores de validación.
...
\begin{tabular}{@{}c@{\hspace{1.5em}}c@{\hspace{1.5em}}>{\raggedright\arraybackslash}p{0.52\textwidth}@{}}
\toprule
...
Se eligió una topología simétrica 64--32--16--32--64 por ser suficientemente expresiva para once variables en CPU y mantener tiempos de inferencia acotados (inferior a un segundo por ciclo en el contenedor de despliegue). Configuraciones con bottleneck más estrecho (8 unidades) y más ancho (32 unidades) se evaluaron en el script \path{tune\_model.py}. La topología base ofreció el mejor equilibrio entre pérdida de validación y estabilidad frente a datos de Prometheus.
...
Los pesos entrenados y la configuración estructural se serializaron por separado (\path{lstm\_autoencoder.weights.h5} y \path{lstm\_autoencoder\_config.json}) para reconstruir el grafo computacional en inferencia sin acoplarse a una versión concreta del entorno de entrenamiento.
...
Sobre estas métricas se derivaron seis variables temporales adicionales mediante codificación cíclica de la hora y del día de la semana, junto con indicadores de fin de semana y horario nocturno. La figura~\ref{fig:dataset-dist} muestra la distribución marginal de las cinco métricas técnicas sobre datos sintéticos ilustrativos con estacionalidad diaria compatible con el \textit{mock service}. Se incluye únicamente con fines demostrativos, dado que no corresponde publicar histogramas de la telemetría real.

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{./Figures/dataset_distribution.png}
\caption{Distribución de las métricas técnicas del servicio (datos sintéticos ilustrativos).}
\label{fig:dataset-dist}
\end{figure}

La tabla~\ref{tab:dataset} sintetiza las características del conjunto antes y después de la transformación a ventanas deslizantes de 20 pasos (10~minutos).
...
El script \path{scripts/train.py} orquestó el flujo completo: carga de configuración, obtención de datos, preprocesamiento, generación de ventanas, entrenamiento, persistencia de artefactos y cálculo del umbral. Los hiperparámetros principales se fijaron en \path{config/model.yaml} y se resumen en la tabla~\ref{tab:hiperparametros}.
...
\item Cálculo del umbral sobre errores de validación y guardado de artefactos en \path{models/}.
\end{enumerate}
...
\midrule
\path{lstm\_autoencoder.weights.h5} & Pesos entrenados del autoencoder \\
\path{lstm\_autoencoder\_config.json} & Arquitectura y metadatos de reconstrucción del grafo \\
\path{preprocessor.joblib} & Escalador \texttt{fixed\_minmax} y columnas activas \\
\path{anomaly\_threshold.npy} & Umbral $\tau$ calibrado en validación \\
\bottomrule
\end{tabular}
\end{table}
...
Tras el entrenamiento, se calcularon los errores $e$ de la ecuación~\eqref{eq:mse-reconstruccion} sobre el conjunto de validación temporal y se definió el umbral $\tau$ como un percentil de esa distribución, configurable en \path{config/alerting.yaml}. El percentil inicial se fijó en un valor alto (99,5) para limitar falsos positivos sobre tramos nominales. El valor definitivo se ajustó iterativamente con el equipo de operaciones durante las pruebas de validación y priorizó una tasa de alertas compatible con la capacidad de respuesta del servicio de guardia.
...
El umbral serializado (\path{models/anomaly\_threshold.npy}) se cargó en inferencia junto con el modelo y el preprocesador. Así se mantuvo coherencia entre el criterio de entrenamiento y el despliegue.

\subsection{Exploración de hiperparámetros}

Se realizó una búsqueda acotada de configuraciones alternativas mediante \path{scripts/tune\_model.py}, que entrenó candidatos en un directorio aislado (\path{evaluation/tuning\_runs/}) sin sobrescribir los artefactos productivos.
...
Los criterios de selección combinaron la pérdida de validación, el margen entre error medio en datos nominales y el umbral propuesto, y la tasa de falsos positivos sobre ventanas sintéticas con anomalías inyectadas. La tabla~\ref{tab:tuning-candidatos} resume los candidatos evaluados; a continuación se comentan los resultados de cada variante.
...
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Candidato} & \textbf{Variante} \\
\midrule
\texttt{baseline} & 64--32--16, ventana 20 \\
\texttt{small\_bottleneck} & 32--16--8 \\
\texttt{wide\_bottleneck} & 128--64--32 \\
\texttt{window\_30} & 30 pasos (15~min) \\
\texttt{dropout\_02} & Dropout 0,2 \\
\bottomrule
\end{tabular}
\end{table}

La configuración \texttt{baseline} (ventana 20, capas 64--32--16, dropout 0,1, stride 1, \texttt{fixed\_minmax}) ofreció el mejor equilibrio entre validación e inferencia en Prometheus y se mantuvo como referencia productiva por combinar simplicidad de despliegue y desempeño estable. La variante \texttt{small\_bottleneck} redujo la capacidad del codificador y tendió a subestimar variaciones legítimas del régimen nominal. \texttt{wide\_bottleneck} incrementó el costo de CPU con una ganancia marginal en la pérdida de validación. Con \texttt{window\_30}, la ventana más larga introdujo mayor inercia y retrasó la detección de incidentes breves. Por último, \texttt{dropout\_02} produjo subajuste y una pérdida de validación más elevada.
...
\begin{enumerate}
\item Sustituir el escalado adaptativo por \texttt{fixed\_minmax}, lo que eliminó el desajuste entre la distribución de entrenamiento y series productivas con distinta escala absoluta.
\item Reducir el stride de ventanas de 20 a 1, lo que aumentó en un factor del orden de 20 la cantidad de ejemplos de entrenamiento y mejoró la representación de patrones circadianos en el espacio latente.
\end{enumerate}

\section{Integración e implementación del sistema}
\label{sec:integracion}

Esta sección describe el servicio de inferencia en contenedor, la gestión de alertas hacia Opsgenie, los enlaces contextuales a Grafana y el despliegue en los entornos de simulación y productivo.

\subsection{Servicio de inferencia}

El servicio de detección (\path{scripts/inference.py}) se ejecutó como proceso en segundo plano dentro de un contenedor Docker basado en \path{python:3.10-slim}. En cada ciclo, configurado cada 30~segundos en \path{config/data.yaml}, el servicio:
...
Para mitigar la fatiga de alertas se configuraron mecanismos en \path{config/alerting.yaml}:
...
El contenedor se construyó con un Dockerfile multi-etapa mínimo: instalación de dependencias Python desde requirements.txt, copia del código fuente y arranque de \path{inference.py} con usuario sin privilegios. Los directorios \path{models/}, \path{config/} y \path{logs/} se montaron como volúmenes para permitir actualizar pesos y umbrales sin reconstruir la imagen. Los límites de recursos (1 CPU virtual y 2~GB de RAM) resultaron suficientes para el ciclo de 30~segundos en las pruebas de despliegue realizadas.
...
\midrule
\path{config/data.yaml} & URL de Prometheus, consultas PromQL, cotas de normalización, intervalos de muestreo \\
\path{config/windowing.yaml} & Tamaño de ventana (20) y stride de entrenamiento (1) \\
\path{config/model.yaml} & Capas del autoencoder, hiperparámetros de entrenamiento \\
\path{config/alerting.yaml} & Método de umbral, credenciales Opsgenie, reglas de deduplicación y enlaces Grafana \\
\bottomrule
\end{tabular}
\end{table}

La tabla~\ref{tab:demo-vs-prod} contrasta el perfil \texttt{dev} de Docker Compose con el despliegue productivo.
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
...
Actualización de modelos & Volumen montado \path{./models} & Montaje equivalente en prod \\
\bottomrule
\end{tabular}
\end{table}

En la organización cliente, únicamente el servicio \texttt{anomaly-detection} se agregó a la infraestructura existente; los servicios auxiliares del perfil \texttt{dev} cumplieron exclusivamente fines de desarrollo y demostración académica.

\subsection{Registro, trazabilidad y requisitos no funcionales}

El servicio registró cada ciclo en \path{logs/} con nivel configurable: operación normal en depuración, anomalías confirmadas en advertencia y fallos de consulta en error. Cuando se detectó una anomalía, el registro incluyó las métricas instantáneas y el promedio sobre la ventana de inferencia. Esos datos complementaron la comparación original vs.\ reconstruida que viaja en el \textit{payload} de Opsgenie.
