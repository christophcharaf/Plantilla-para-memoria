...
La tabla~\ref{tab:pipeline-etapas} detalla entradas, salidas y artefactos de cada etapa, la figura \ref{fig:sliding_window} demuestra gráficamente la ventana deslizante.



\begin{figure}
\centering
\includegraphics[width=0.92\textwidth]{./Figures/sliding_window.png}
...
Normalización & Crudos $\rightarrow$ $[0,1]$ fijo & \texttt{MinMaxScaler}, cotas YAML \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{./Figures/infra_demo.png}
...
Las etapas \texttt{Encoder LSTM} y \texttt{Decoder LSTM} implementan el esquema codificador--\textit{bottleneck}--decodificador de Malhotra et al. en el módulo \texttt{LSTMAutoencoder}, adaptado al vector de once variables y a la ventana de 20 pasos.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{./Figures/flow_diag.png}
...
El decodificador expandió el latente mediante \texttt{RepeatVector}, replicándolo a lo largo de los 20 pasos temporales, y aplicó tres capas \texttt{LSTM} simétricas (16, 32 y 64 unidades) con dropout del 10~\%. La salida se obtuvo con una capa \texttt{TimeDistributed}\allowbreak\texttt{(Dense(11))} que reconstruyó las once variables en cada instante. La figura~\ref{fig:lstm-autoencoder-arch} resume la pila de capas y las formas tensoriales intermedias.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{./Figures/lstm_autoencoder_arch.png}
...
El contenedor se construyó con un \path{Dockerfile} multi-etapa mínimo: instalación de dependencias Python desde \path{requirements.txt}, copia del código fuente y arranque de \path{inference.py} con usuario sin privilegios. Los directorios \path{models/}, \path{config/} y \path{logs/} se montaron como volúmenes para permitir actualizar pesos y umbrales sin reconstruir la imagen. Los límites de recursos (1 CPU virtual y 2~GB de RAM) resultaron suficientes para el ciclo de 30~segundos en las pruebas de despliegue realizadas.
