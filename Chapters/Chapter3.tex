...
\midrule
\path{src/data/prometheus\_client.py} & Consultas HTTP, agregación y alineación de series \\
\path{src/data/preprocessor.py} & Variables temporales y normalización \\
\path{src/data/windowing.py} & Ventanas deslizantes para entrenamiento e inferencia \\
\path{src/data/synthetic\_data.py} & Generación offline de series de respaldo \\
\path{src/models/lstm\_autoencoder.py} & Definición, entrenamiento y persistencia del modelo \\
\path{src/alerting/detector.py} & Puntuación de anomalía por ventana \\
\path{src/alerting/opsgenie\_client.py} & Emisión de alertas y notificaciones de resolución \\
\path{src/alerting/grafana\_links.py} & Construcción de URLs contextuales al panel de Grafana \\
\path{scripts/train.py} & Orquestación del entrenamiento extremo a extremo \\
\path{scripts/inference.py} & Bucle de detección en tiempo de operación \\
\path{scripts/tune\_model.py} & Exploración de arquitecturas sin afectar artefactos productivos \\
\bottomrule
\end{tabular}
\end{table}
...
Consultas PromQL & \path{config/data.yaml} & Mismo archivo \\
...
A continuación, en la figura \ref{fig:infra_demo.png} se ilustra la arquitectura del sistema desarrollado, desde el servicio simulado en Flask hasta la visualización y alertas por Grafana y Opsgenie respectivamente. 

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{./Figures/infra_demo.png}
\caption{Estructura del sistema de detección.}
\label{fig:infra_demo.png}
\end{figure}

\subsection{Codificador y decodificador}

La arquitectura implementada en \texttt{LSTMAutoencoder} (\path{src/models/lstm\_autoencoder.py}) siguió el esquema codificador--bottleneck--decodificador de Malhotra et al., adaptado al vector de once variables y a la ventana de 20 pasos.
